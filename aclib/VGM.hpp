...
struct VGM_YM2203 : public VGM_YM2149, public OPN {
	...
	void Calc_VGM_Time_150(size_t event_Time)
	{
		if (event_Time - this->Time_Prev) {
			// Tqn = 60 / Tempo
			// TPQN = 48
			...
			// ここの定数部分は故意にわかりやすくするために約分せずに記述し、C++17のstd::gcdを通している。(N / D = )
			constexpr size_t N = 3 * 60 * VGM_CLOCK * 2;
			constexpr size_t D = 2 * 48;
			constexpr size_t gcd_VGMT = std::gcd(N, D);
			size_t N_VGMT = event_Time * (N / gcd_VGMT);
			size_t D_VGMT = this->Tempo * (D / gcd_VGMT);
...
struct VGM_YM2608 : public virtual VGM_YM2203, public OPNA {
	OPNSSGVOL ex_vgm;
...
struct VGM_YM2608_MAKO2 : public VGM_YM2608, public VGM_YM2203_MAKO2, public OPNA_MAKO2 {
	...
	void Key_set_LFO_FM2(unsigned __int8 CH, __int16 Detune)
	{
		union {
			struct {
				unsigned __int16 FNumber : 11;
