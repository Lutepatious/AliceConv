...
void make_vgm_init(unsigned __int8** dest, enum CHIP chip)
{
	...
	memcpy_s(*dest, Init_len, Init, Init_len);
	*dest += Init_len;
}
...
void pcm2wav(unsigned __int8* pcmdata, wchar_t* filename, size_t filelen)
{
	size_t pcm_size;
	size_t pcm_srate;
	size_t wcount;
	...
	if (*((unsigned __int64*)pcmdata + 2) == 0 && *((unsigned __int32*)pcmdata + 3)) {
		...
		if (*(unsigned __int32*)pcmdata == _byteswap_ulong(0x4D506400)) {
			...
			pcm_inbuf = pcmdata + sizeof(struct PM_header);
		}
		else {
...
//				unsigned __int8* src = &inbuf[*(pBlock_offset + j)];
		...
		while ((pEVENTs + length_real)->time < end_time) {
			if ((pEVENTs + length_real)->time >= time_loop_start) {
		...
		if (loop_start_index != SIZE_MAX) {
			while (loop_start_index > 0
				&& (pEVENTs + length_real)->CH == (pEVENTs + loop_start_index)->CH
				&& (pEVENTs + length_real)->Event == (pEVENTs + loop_start_index)->Event
				&& (pEVENTs + length_real)->Param.B[0] == (pEVENTs + loop_start_index)->Param.B[0]
				&& (pEVENTs + length_real)->time - (pEVENTs + length_real - 1)->time == (pEVENTs + loop_start_index)->time - (pEVENTs + loop_start_index - 1)->time
				)
			{
				length_real--;
				loop_start_index--;
...
//			Ex_Vols_count = 1;
		...
		for (struct EVENT* src = pEVENTs; (src - pEVENTs) <= length_real; src++) {
			...
			if (loop_enable && (src - pEVENTs) == loop_start_index) {
	...
	//				wprintf_s(L"%8zu: OLD tempo %zd total %zd\n", src->time, Tempo, Time_Prev_VGM);
		...
		*vgm_pos++ = 0x66;
		...
		wchar_t* path = filename_replace_ext(*argv, L".vgm");
