...
int wmain(int argc, wchar_t** argv)
{
	...
	while (--argc) {
		...
		if (**++argv == L'-') {
			...
			else if (*(*argv + 1) == L'T') { // “¬_“sŽs X68000 256F
				dm = decode_mode::X68T;
			}
			else if (*(*argv + 1) == L'B') { // RanceIII ƒIƒvƒVƒ‡ƒ“ƒZƒbƒg ‚ ‚Ô‚È‚¢•¶‰»Õ‘O–é X68000 256F
				dm = decode_mode::X68B;
			}
			else if (*(*argv + 1) == L'p') { // ‹ŒPMS(VSP256)
				dm = decode_mode::VSP256;
			}
			else if (*(*argv + 1) == L'k') { // PMS8 ‹S’{‰¤ƒ‰ƒ“ƒXˆÈ~
				dm = decode_mode::PMS8;
			}
			else if (*(*argv + 1) == L'i') { // PMS16 ‚¢‚¯‚È‚¢‚©‚Â‚Ýæ¶
