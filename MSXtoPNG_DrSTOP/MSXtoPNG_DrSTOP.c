...
int wmain(int argc, wchar_t** argv)
{
	...
	while (--argc) {
		...
		for (size_t y = 0; y < msxvsp_len_y / 2; y++) {
			memcpy_s(msx_data_deinterlaced + msxvsp_len_col * y * 2, msxvsp_len_col, msx_data_decoded + msxvsp_len_col * y, msxvsp_len_col);
			memcpy_s(msx_data_deinterlaced + msxvsp_len_col * (y * 2 + 1), msxvsp_len_col, msx_data_decoded + msxvsp_len_col * (msxvsp_len_y / 2 + y), msxvsp_len_col);
		}
