...
struct X68V_header {
	unsigned __int16 Start_Col;
	unsigned __int16 Start_Row;
	unsigned __int16 Cols;
	unsigned __int16 Rows;
	unsigned __int16 Pal[0x100]; // Palette No. 0x00 to 0xFE
} hX68V;
...
int wmain(int argc, wchar_t** argv)
{
	...
	while (--argc) {
		...
		size_t x68_in_x = _byteswap_ushort(hX68V.Start_Col) * 3;
		size_t x68_in_y = _byteswap_ushort(hX68V.Start_Row) * 3;
